...
describe AsakusaSatellite::Filter::JiraTicketLink do
  describe "API Keyなし" do
    context "JIRA の URL の最後が /" do
      before do
        @config = { :room =>
          OpenStruct.new(:yaml => {
              :jira_ticket => { 'root' => 'http://jira.example.com/a/'
              }})
        }
        @filter = AsakusaSatellite::Filter::JiraTicketLink.new({})
      end
      ...
      it {
        should == 'foo <a target="_blank" href="http://jira.example.com/a/issues/223">#223</a>'
      }
    end

    context "JIRA の URL の最後が / じゃない" do
      before do
        @config = { :room =>
          OpenStruct.new(:yaml => {
              :jira_ticket => { 'root' => 'http://jira.example.com/a'
...
JSON
      ...
      @filter.stub!(:open).with('http://jira.example.com/a/issues/380.json?key=hoge').and_yield(io)
    end
    ...
    it {
      should == '<a target="_blank" href="http://jira.example.com/a/issues/380">#380 Jira プラグインをタイトルも取れるようにする</a>'}
  end
end
